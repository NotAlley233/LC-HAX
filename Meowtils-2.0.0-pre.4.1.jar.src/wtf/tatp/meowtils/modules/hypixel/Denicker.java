...
/*     */           ScorePlayerTeam team = mc.field_71441_e.func_96441_U().func_96509_i(player.func_70005_c_());
/*     */           if (team != null) {
/*     */             String colorPrefix = team.func_96668_e();
/*     */             if (!GamemodeUtil.bedwarsGame || GamemodeUtil.bedwarsLobby) {
/*     */               return;
/*     */             }
/*     */             if (hasAlerted)
/*     */               return; 
/*     */             if (colorPrefix.contains("§a")) {
/*     */               Meowtils.addMessage("Highest rank in the lobby: " + EnumChatFormatting.GREEN + "[VIP]" + EnumChatFormatting.DARK_GRAY + " | " + EnumChatFormatting.GREEN + "[VIP" + EnumChatFormatting.GOLD + "+" + EnumChatFormatting.GREEN + "]");
/*     */             } else if (colorPrefix.contains("§b")) {
/*     */               Meowtils.addMessage("Highest rank in the lobby: " + EnumChatFormatting.AQUA + "[MVP]" + EnumChatFormatting.DARK_GRAY + " | " + EnumChatFormatting.AQUA + "[MVP" + EnumChatFormatting.RED + "+" + EnumChatFormatting.AQUA + "]");
/*     */             } else if (colorPrefix.contains("§6")) {
/*     */               Meowtils.addMessage("Highest rank in the lobby: " + EnumChatFormatting.GOLD + "[MVP" + EnumChatFormatting.RED + "++" + EnumChatFormatting.GOLD + "]");
/*     */             } else if (colorPrefix.contains("§c")) {
/*     */               Meowtils.addMessage("Highest rank in the lobby: " + EnumChatFormatting.RED + "[" + EnumChatFormatting.WHITE + "YOUTUBE" + EnumChatFormatting.RED + "]" + EnumChatFormatting.DARK_GRAY + " | " + EnumChatFormatting.RED + "[" + EnumChatFormatting.GOLD + "ዞ" + EnumChatFormatting.RED + "]");
/*     */             } 
/*     */             hasAlerted = true;
/*     */           } 
/*  81 */         })).start();
...
/*  85 */     if (event.phase != TickEvent.Phase.END || this.mc.field_71441_e == null || this.mc.field_71439_g == null)
/*  86 */       return;  if (GamemodeUtil.megaWalls)
/*     */       return; 
/*  88 */     for (NetworkPlayerInfo npi : this.mc.func_147114_u().func_175106_d()) {
/*  89 */       GameProfile profile = npi.func_178845_a();
/*  90 */       String uuid = profile.getId().toString();
/*     */       
/*  92 */       if (uuid.length() < 15 || uuid.charAt(14) != '1')
/*     */         continue; 
/*  94 */       String name = profile.getName();
/*  95 */       if (parsed.contains(name))
/*     */         continue; 
/*  97 */       String displayName = (npi.func_178854_k() != null) ? npi.func_178854_k().func_150260_c() : name;
/*  98 */       boolean hasColor = displayName.contains("§");
/*     */       
/* 100 */       if (!hasColor) {
/* 101 */         int ticks = ((Integer)noColorTicks.merge(name, Integer.valueOf(1), Integer::sum)).intValue();
/* 102 */         if (ticks < 10)
/*     */           continue; 
/*     */       } 
/* 105 */       Pair<String, String> skinData = getSkinData(profile);
/* 106 */       if (skinData != null) {
/* 107 */         String skinHash = (String)skinData.getKey();
/* 108 */         String realName = (String)skinData.getValue();
/* 109 */         String cleanDisplayName = cleanName(displayName);
/* 110 */         alert(realName, cleanDisplayName, skinHash);
/* 111 */         parsed.add(name);
/* 112 */         noColorTicks.remove(name);
/*     */       } 
/*     */     } 
/*     */   }
/*     */   
/*     */   public static void alert(String name, String displayName, String hash) {
/* 118 */     String localPlayer = (Minecraft.func_71410_x()).field_71439_g.func_70005_c_();
/* 119 */     if (nicks.contains(hash)) {
/* 120 */       Meowtils.addMessage(EnumChatFormatting.RED + displayName + EnumChatFormatting.DARK_PURPLE + " is nicked.");
/* 121 */       if (cfg.v.notify && cfg.v.notifyDenicker) {
/* 122 */         NotifyUtil.notifyPartyQueue(displayName + " is nicked.");
/*     */       }
/*     */     } else {
/* 125 */       if (name.equals(localPlayer))
/* 126 */         return;  Meowtils.addMessage(EnumChatFormatting.GOLD + name + EnumChatFormatting.DARK_PURPLE + " is nicked as " + EnumChatFormatting.RED + displayName + EnumChatFormatting.DARK_PURPLE + ".");
/* 127 */       if (cfg.v.notify && cfg.v.notifyDenicker) {
/* 128 */         NotifyUtil.notifyPartyQueue(name + " is nicked as " + displayName + ".");
/*     */       }
/*     */     } 
/*     */   }
/*     */   
/*     */   private String cleanName(String name) {
/* 134 */     while (name.matches(".*§[0-9a-frk-o]$")) {
/* 135 */       name = name.substring(0, name.length() - 2);
/*     */     }
/* 137 */     return name.trim();
/*     */   }
/*     */   public static Pair<String, String> getSkinData(GameProfile profile) {
/* 140 */     Property property = (Property)Iterables.getFirst(profile.getProperties().get("textures"), null);
/* 141 */     if (property == null) return null;
/*     */     
/*     */     try {
/* 144 */       String decoded = new String(Base64.getDecoder().decode(property.getValue()), StandardCharsets.UTF_8);
/* 145 */       JsonObject obj = (new JsonParser()).parse(decoded).getAsJsonObject();
/*     */       
/* 147 */       if (!obj.has("textures")) return null; 
/* 148 */       JsonObject textures = obj.getAsJsonObject("textures");
/*     */       
/* 150 */       if (!textures.has("SKIN")) return null; 
/* 151 */       JsonObject skin = textures.getAsJsonObject("SKIN");
/*     */       
/* 153 */       if (!skin.has("url")) return null; 
/* 154 */       String url = skin.get("url").getAsString();
/* 155 */       String hash = url.substring(url.lastIndexOf('/') + 1);
/*     */       
/* 157 */       String realName = obj.has("profileName") ? obj.get("profileName").getAsString() : "";
/*     */       
/* 159 */       return new Pair(hash, realName);
...
/* 165 */     parsed.clear();
/* 166 */     noColorTicks.clear();
/* 167 */     tickCounter = 0;
/* 168 */     lastAlertTime = 0L;
/* 169 */     hasAlerted = false;
/*     */   }
/*     */ }


/* Location:              /Users/notalley/Downloads/Meowtils-2.0.0-pre.4.1.jar!/wtf/tatp/meowtils/modules/hypixel/Denicker.class
